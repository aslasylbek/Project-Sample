...
class ReadingTaskViewController: UIViewController {
    ...
    func rebuildUIForTrueFalse() {
        ...
        btnTrue = UIButton()
        btnTrue!.backgroundColor = UIColor(named: "primary")
        ...
        btnFalse = UIButton()
        btnFalse!.backgroundColor = UIColor(named: "primary")
        ...
        btnNot = UIButton()
        btnNot!.backgroundColor = UIColor(named: "primary")
        btnNot!.setTitle("Not Given", for: .normal)
        btnNot!.addTarget(self, action: #selector(buttonAction), for: .touchUpInside)
        self.svAnswer.addArrangedSubview(btnNot!)
    ...
    @objc func buttonAction(sender: UIButton!) {
        ...
        if sender.tag != 0{
            ...
            if let correctBtn = svAnswer.viewWithTag(Int((trueFalse?.id)!)!) as? UIButton{
                correctBtn.backgroundColor = UIColor(named: "correct")
            }
        }
    ...
    func updateTrueFalse() {
        if questionNumber < (allReading?.truefalse?.count)!{
            ...
            else if trueFalse?.truefalse! == "2"{
                btnNot?.tag = Int((trueFalse?.id)!)!
            }
            updateUI()
        }
        else{
